...
from transformers import pipeline  # PyTorch-based NER

# Initialize EasyOCR reader
reader = easyocr.Reader(['en'])

# Load PyTorch model for NER
...
def extract_text_by_color(image, mask):
    """Enhanced text extraction with better accuracy using EasyOCR."""
...
def extract_and_format_text(image_path):
    ...
    # Apply PyTorch model for NER
    ner_results = ner_model(list(final_result.values()))

    # Process the NER results if necessary
    for ner in ner_results:
        if ner['word'] not in final_result:
            final_result[ner['word']] = ner['entity']  # Adjust as needed
